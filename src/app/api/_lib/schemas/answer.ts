...
export const answerSchema = z.object({
  id: z.number().int().positive().max(INT32_MAX).describe('Answer id'),
...
export const answerCreateSchema = z.object({
  questionId: z.coerce.number().int().positive().max(INT32_MAX).describe('Related question id'),
  text: z.string().min(1).describe('Answer text'),
  isCorrect: z.boolean().default(false).describe('Marks if answer is correct'),
  orderIdx: z.coerce.number().int().min(0).max(INT32_MAX).default(0).describe('Ordering index (>=0)'),
});
...
export const answerUpdateSchema = z.object({
  ...
  orderIdx: z.coerce.number().int().min(0).max(INT32_MAX).optional().describe('Updated order index (>=0)'),
});
