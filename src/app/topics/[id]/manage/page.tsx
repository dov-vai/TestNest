...
export default function ManageTopicPage() {
  ...
  const [newQuestion, setNewQuestion] = useState({ text: '', type: 'single' });
  const [newAnswer, setNewAnswer] = useState({ text: '', isCorrect: false }); // Simple answer creation for demo
  // Managing answers for new question is complex (multiple answers).
  // Simplified: Create question first, then add answers?
  // Or just support simple "Select Existing" for now as per "Add existing questions" spec.
  // Spec also says "Create new tests" which implies creating questions.
  ...
  const availableToLink = availableQuestions.filter(q => 
      !topicQuestions.some(tq => tq.question.id === q.id)
  );
            ...
            <ul className="divide-y divide-gray-200">
                {topicQuestions.length === 0 ? (
                    <li className="px-4 py-12 text-center text-gray-500">
                        No questions in this topic yet. Add some!
                    </li>
                ) : (
            ...
            <div className="space-y-4">
                    ...
                    >
                        ...
                        {availableToLink.map(q => (
                            <option key={q.id} value={q.id}>
                                {q.text.substring(0, 50)}{q.text.length > 50 ? '...' : ''} ({q.type})
                            </option>
                        ))}
                    </select>
                ...
                <div className="pt-4 border-t border-gray-200">
                     <p className="text-sm text-gray-500 text-center">
                         (To create new questions, use the Create Question page - not implemented in this demo view)
                     </p>
                </div>
            </div>
        </Modal>
